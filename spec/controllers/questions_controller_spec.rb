...
RSpec.describe QuestionsController, vcr: true do
  describe 'GET show' do
    context 'with an answer' do
      before(:each) do
        get :show, params: { question_id: 'YTrFU4cX' }
      end
    ...
    context 'with no answer' do
      before(:each) do
        ...
        get :show, params: { question_id: 'dQuMWNeE' }
      end

      it 'does not assign @answering_person' do
        expect(assigns(:answering_person)).to eq(nil)
      end
    end
  end
