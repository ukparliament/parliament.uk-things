...
RSpec.describe 'questions/show', vcr: true do
  context 'Question has been answered' do
    ...
    context 'answering person government_position' do
      context 'answering person has no role' do
        before(:each) do
          assign(:answering_person_government_incumbency, nil)
          assign(:government_position_name, nil)

          render
        end

        it 'will render the correct asking person house' do
          expect(rendered).not_to match(/until 24 January 2018/)
        end
      end
    end
  end
